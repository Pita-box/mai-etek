...
const isDom = async (req: any, res: any, next: any) => {
  try {
...
router.get('/vault/reveal/:userId', isDom, async (req: any, res: any) => {
  try {
    ...
    const password = decrypt(vaultData.encrypted_password);
    res.json({ password });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/users', isDom, async (req: any, res: any) => {
  try {
    const domId = req.user.id;
...
router.post('/claim/:subId', isDom, async (req: any, res: any) => {
  try {
...
router.patch('/config/:subId', isDom, async (req: any, res: any) => {
  try {
    const { subId } = req.params;
    const domId = req.user.id;
