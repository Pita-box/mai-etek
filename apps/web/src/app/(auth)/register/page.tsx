...
export default function RegisterPage() {
  ...
  async function onSubmit(data: RegisterFormValues) {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/superadmin/register`, {
        method: "POST",
        headers: {
      ...
      setError(err instanceof Error ? err.message : "Registrace se nepodařila.")
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="w-full max-w-md space-y-8 p-8 rounded-2xl glass-card glass-card-hover z-10">
        <div className="text-center">
          ...
          <p className="text-sm text-muted mt-2">
            Zadej svoje údaje pro registraci
          </p>
        </div>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <p className="text-xs text-muted">
                    Nepovinné — když ho nevyplníš, nastavím jméno subíček.
                  </p>
